...
clim_extract <- function(wth_data, ini.date){
  ...
  wth_data %>% mutate(across(.cols = c(day, month), as.numeric)) %>%
  group_by(day, month) %>%
...
wth_escenaries <-  Resam %>%
  mutate(Escenaries = map(Escenaries, ~.x %>%
                            mutate(data = map(data,
                                              ~.x %>%
                                                mutate(data = map(data,
                                                                  ~.x %>% mutate(across(.cols = c(day, month), as.integer)))))))) %>%
...
wth_escenaries <- wth_escenaries %>%
  mutate(wth_final = pmap(.l = list(Escenaries, chirp_data, nasa_data, climatology_mean),
                          .f = join_wth_escenarios))

# wth_escenaries <-  Resam %>%  sample_n(10) %>% 
#   mutate(chirp_data = map(coord, ~extract_chirp_safety(path_Chirp, .x)),
#          nasa_data  = map2(stations, coord, ~download_nasa_safety(ini.date, end.date, .x, .y)),
#          climatology_mean = map(stations, ~clim_extract(.x, ini.date)),
